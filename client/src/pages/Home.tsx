...
import { DEFAULT_SUBTITLE_POSITION, getSubtitlePreviewPosition } from "@shared/subtitlePosition";
...
export default function Home() {
  ...
  const selectedSubtitle = editorState.subtitles.find(cue => cue.id === selectedSubtitleId) ?? null;
  ...
  const subtitleDragControllerRef = useRef<ReturnType<typeof createTextDragController> | null>(null);
  if (!subtitleDragControllerRef.current) {
    subtitleDragControllerRef.current = createTextDragController({
      getBounds: () => previewRef.current?.getBoundingClientRect(),
      selectText: setSelectedSubtitleId,
      setDraggingText: setDraggingSubtitleId,
      updatePosition: (id, position) => setEditorState(state => ({
        ...state,
        subtitles: state.subtitles.map(cue => cue.id === id ? { ...cue, ...position } : cue),
      })),
    });
  }
  const beginSubtitleDrag = (event: ReactPointerEvent<HTMLDivElement>, id: string) => {
    if (editingSubtitleId === id) return;
    subtitleDragControllerRef.current?.begin(event, id);
  };
  const dragSubtitle = (event: ReactPointerEvent<HTMLDivElement>, id: string) => subtitleDragControllerRef.current?.move(event, id);
  const endSubtitleDrag = (event: ReactPointerEvent<HTMLDivElement>, id: string) => subtitleDragControllerRef.current?.end(event, id);
  ...
  const createSubtitle = () => {
    const cue: SubtitleCue = { id: makeId("sub"), startMs: currentMs, endMs: Math.min(timelineDurationMs, currentMs + 2_500), text: "在此輸入字幕", ...DEFAULT_SUBTITLE_POSITION };
        ...
        <div className="grid min-w-0 grid-rows-[minmax(420px,1fr)_300px] xl:grid-cols-[minmax(0,1fr)_310px] xl:grid-rows-[minmax(0,1fr)_300px]">
          <section className="relative flex min-h-[420px] min-w-0 flex-col items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_50%_35%,#202039_0%,#12121b_42%,#0d0d12_72%)] p-4 xl:col-start-1 xl:row-start-1">
            ...
            <div ref={previewRef} className="relative max-h-[calc(100%-4rem)] max-w-full overflow-hidden rounded-2xl bg-[#050507] shadow-[0_28px_70px_rgba(0,0,0,0.55)]" style={{ aspectRatio: aspectRatio === "16:9" ? "16 / 9" : "9 / 16", height: aspectRatio === "16:9" ? "min(52vw, 470px)" : "min(52vh, 470px)" }}>
              ...
              <div className="pointer-events-none absolute inset-0 overflow-hidden">
                ...
                {visibleSubtitles.map(cue => {
                  const position = getSubtitlePreviewPosition(cue);
                  const editing = editingSubtitleId === cue.id;
                  return <div key={cue.id} contentEditable={editing} suppressContentEditableWarning onPointerDown={event => beginSubtitleDrag(event, cue.id)} onPointerMove={event => dragSubtitle(event, cue.id)} onPointerUp={event => endSubtitleDrag(event, cue.id)} onPointerCancel={event => endSubtitleDrag(event, cue.id)} onDoubleClick={event => { event.stopPropagation(); setSelectedSubtitleId(cue.id); setEditingSubtitleId(cue.id); }} onBlur={event => { if (editing) updateSubtitle(cue.id, { text: event.currentTarget.innerText }); setEditingSubtitleId(null); }} className={`pointer-events-auto absolute max-w-[82%] -translate-x-1/2 -translate-y-1/2 rounded bg-black/70 px-3 py-1.5 text-center text-xs font-medium leading-relaxed text-white shadow-lg touch-none outline-none md:text-sm ${draggingSubtitleId === cue.id ? "cursor-grabbing" : editing ? "cursor-text" : "cursor-grab"} ${selectedSubtitleId === cue.id ? "ring-1 ring-violet-300 ring-offset-2 ring-offset-black/30" : ""}`} style={getSubtitlePreviewStyle(cue)} title="拖曳以移動字幕；雙擊可直接編輯">{cue.text}</div>;
                })}
                {selectedSubtitle && visibleSubtitles.some(cue => cue.id === selectedSubtitle.id) && <div className="absolute bottom-3 right-3 z-20 w-44 rounded-lg border border-violet-300/30 bg-black/85 p-2.5 text-left shadow-xl backdrop-blur"><p className="mb-2 text-[10px] font-medium text-violet-200">預覽區字幕時間</p><div className="grid grid-cols-2 gap-1.5"><label className="text-[9px] text-zinc-400">開始<input aria-label="預覽字幕開始秒數" type="number" min="0" step="0.1" value={(selectedSubtitle.startMs / 1000).toFixed(1)} onPointerDown={event => event.stopPropagation()} onChange={event => updateSubtitle(selectedSubtitle.id, updateSubtitleTiming(selectedSubtitle, { startMs: Number(event.target.value) * 1000 }))} className="mt-1 h-7 w-full rounded border border-white/15 bg-white/[0.07] px-1 text-[11px] text-white outline-none focus:border-violet-300" /></label><label className="text-[9px] text-zinc-400">結束<input aria-label="預覽字幕結束秒數" type="number" min="0" step="0.1" value={(selectedSubtitle.endMs / 1000).toFixed(1)} onPointerDown={event => event.stopPropagation()} onChange={event => updateSubtitle(selectedSubtitle.id, updateSubtitleTiming(selectedSubtitle, { endMs: Number(event.target.value) * 1000 }))} className="mt-1 h-7 w-full rounded border border-white/15 bg-white/[0.07] px-1 text-[11px] text-white outline-none focus:border-violet-300" /></label></div></div>}
              </div>
          ...
          <aside className="z-10 min-w-0 overflow-y-auto border-l border-white/[0.08] bg-[#111118] xl:col-start-2 xl:row-start-1">
            ...
            <div className="p-4">
              ...
              {activeTool === "captions" && <div className="space-y-4"><div className="rounded-xl border border-violet-400/20 bg-violet-400/[0.06] p-3"><div className="flex items-start gap-2"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-violet-300" /><div><h3 className="text-xs font-medium text-violet-100">AI 一鍵字幕</h3><p className="mt-1 text-[11px] leading-4 text-zinc-400">Whisper 會從處理過的音訊產生可編輯、帶時間戳的字幕。可在預覽區拖曳，雙擊直接改字。</p></div></div><Button size="sm" disabled={!editorState.clips.length || transcribeCaptions.isPending} onClick={recognizeCaptions} className="mt-3 w-full bg-violet-500 text-white hover:bg-violet-400">{transcribeCaptions.isPending ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <Mic2 className="mr-1.5 h-3.5 w-3.5" />}{transcribeCaptions.isPending ? "Whisper 辨識中" : "開始辨識"}</Button></div><div className="flex items-center justify-between"><span className="text-xs font-medium text-zinc-300">字幕片段</span><div className="flex gap-1"><Button variant="ghost" size="icon" onClick={createSubtitle} className="h-7 w-7 text-zinc-300 hover:bg-white/10"><Plus className="h-4 w-4" /></Button><Button variant="ghost" size="icon" onClick={downloadSrt} className="h-7 w-7 text-zinc-300 hover:bg-white/10"><FileText className="h-3.5 w-3.5" /></Button></div></div><div className="space-y-2">{editorState.subtitles.length ? editorState.subtitles.map((cue, index) => { const position = getSubtitlePreviewPosition(cue); return <div key={cue.id} onClick={() => { setSelectedSubtitleId(cue.id); seekTo(cue.startMs); }} className={`rounded-xl border p-2.5 ${selectedSubtitleId === cue.id ? "border-violet-300/60 bg-violet-400/[0.09]" : "border-white/[0.07] bg-white/[0.025]"}`}><div className="mb-2 flex items-center justify-between"><span className="font-mono text-[10px] text-indigo-300">{String(index + 1).padStart(2, "0")} · {formatTime(cue.startMs)}</span><button onClick={event => { event.stopPropagation(); updateState("subtitles", editorState.subtitles.filter(item => item.id !== cue.id)); if (selectedSubtitleId === cue.id) setSelectedSubtitleId(null); }} className="text-zinc-600 hover:text-red-300"><Trash2 className="h-3.5 w-3.5" /></button></div><textarea value={cue.text} onClick={event => event.stopPropagation()} onChange={event => updateSubtitle(cue.id, { text: event.target.value })} rows={2} className="w-full resize-none border-0 bg-transparent text-xs leading-5 text-zinc-200 outline-none" /><div className="mt-2 grid grid-cols-2 gap-2"><label className="text-[10px] text-zinc-500">開始<input type="number" min="0" value={Math.round(cue.startMs / 1000)} onClick={event => event.stopPropagation()} onChange={event => updateSubtitle(cue.id, { startMs: Number(event.target.value) * 1000 })} className="mt-1 h-7 w-full rounded border border-white/10 bg-black/20 px-1.5 text-xs text-zinc-300" /></label><label className="text-[10px] text-zinc-500">結束<input type="number" min="0" value={Math.round(cue.endMs / 1000)} onClick={event => event.stopPropagation()} onChange={event => updateSubtitle(cue.id, { endMs: Number(event.target.value) * 1000 })} className="mt-1 h-7 w-full rounded border border-white/10 bg-black/20 px-1.5 text-xs text-zinc-300" /></label><label className="text-[10px] text-zinc-500">水平<input type="number" min="0" max="100" value={position.x} onClick={event => event.stopPropagation()} onChange={event => updateSubtitle(cue.id, { x: Number(event.target.value) })} className="mt-1 h-7 w-full rounded border border-white/10 bg-black/20 px-1.5 text-xs text-zinc-300" /></label><label className="text-[10px] text-zinc-500">垂直<input type="number" min="0" max="100" value={position.y} onClick={event => event.stopPropagation()} onChange={event => updateSubtitle(cue.id, { y: Number(event.target.value) })} className="mt-1 h-7 w-full rounded border border-white/10 bg-black/20 px-1.5 text-xs text-zinc-300" /></label></div></div>; }) : <p className="rounded-lg bg-white/[0.025] p-4 text-center text-xs text-zinc-500">尚未建立字幕。可使用 AI 辨識，或按「＋」手動新增。</p>}</div><p className="rounded-lg border border-violet-400/10 bg-violet-400/[0.04] p-2.5 text-[11px] leading-5 text-zinc-400">字幕座標與內容會隨專案儲存。按「開始渲染輸出」時，會以相同位置燒錄進 MP4。</p><Button variant="outline" onClick={downloadSrt} className="w-full border-white/10 bg-white/[0.04] text-zinc-200 hover:bg-white/[0.08]"><Download className="mr-1.5 h-3.5 w-3.5" />下載 SRT 字幕檔</Button></div>}
